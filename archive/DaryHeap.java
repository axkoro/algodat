...
public class DaryHeap<T extends Comparable<T>> {
	...
	public void build(List<T> list) {
		...
		int lastParentNode = (size() - 1) / d; // vorletzte Ebene (siftDown wäre bei Blättern trivial)
		for (int pos = lastParentNode; pos >= 0; pos--) { // Bottom-up-sift-down
			siftDown(pos);
		}
	}
	...
	protected void siftUp(int i) {
		...
		int parent = (i-1) / d;
		if (elements.get(parent).compareTo(elements.get(i)) > 0) {
			swap(parent, i);
			siftUp(parent);
		}
	}
	...
	/**
	 * Methode zum Hinzufügen des neuen Elements element zum heapgeordneten Array.
	 * 
	 * @param element
	...
	public List<T> smallerAs(T element) {
		...
		smallerAsSubtree(elementsSmaller, 0, element);
		return elementsSmaller;
	}

	/**
	 * Hilfsmethode, die diejenigen Unterbäume eines Knotens im heapgeordneten Array zu einer übergebenen Liste hinzufügt, in denen jedes Element kleiner ist als ein übergebenes Element.
	 * @param elementsSmaller Liste, die nach Ausführung die gefundenen Elemente enthält
	 * @param pos Index des Elements, dessen Unterbäume überprüft werden sollen
	 * @param element das Element, das mit Elementen im heapgeordneten Array verglichen werden soll
	 */
	private void smallerAsSubtree(List<T> elementsSmaller, int pos, T element) {
		if (pos >= size()) return;

		if (elements.get(pos).compareTo(element) < 0) {
			elementsSmaller.add(elements.get(pos));

			boolean isBlatt = pos > (size()-1) / d;
			if (isBlatt) return;

			for (int i = 1; i <= d; i++) { // Überprüfe Kinder rekursiv
				int child = pos * d + i;
				smallerAsSubtree(elementsSmaller, child, element);
			}
		}
	}
